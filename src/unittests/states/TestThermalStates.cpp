...
void TestState::TestThermalState()
{
    ...
    TS_ASSERT_THROWS_EQUALS( thermalState.AddTemperature( 1.3, 0.0 ), std::runtime_error & e,
                             strcmp( e.what(), "Volume must be bigger than zero.\n" ), 0 );
    TS_ASSERT_THROWS_EQUALS( thermalState.GetPowerDissipation( 6.0, 3.01 ), std::runtime_error & e,
                             strcmp( e.what(),
                                     "Time passed for GetPowerDissipation() must be equal or later than starting time "
                                     "of last electrical simulation.\n" ),
                             0 );
    /*
        ...
        TS_ASSERT_THROWS_EQUALS(thermalState.GetPowerDissipation(6.0, 3.01), const std::runtime_error &e,
                                CmpUnexp(e, "Time passed for GetPowerDissipation() must be equal or later than starting
       time of last electrical simulation", "GetPowerDissipation"), 0);
    */

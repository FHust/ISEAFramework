...
void TestElectricalElement::testElectricalElementOhmic2DLookup()
{
    ...
    electrical::OhmicResistance< myMatrixType > test(new object::LookupObj2D<double>( data, measurementpointRow, measurementpointColumn ) );
    myMatrixType current;
    current.zeros(1,2);
...
void TestElectricalElement::testElectricalElementVoltageSource2DLookup()
{
    ...
    electrical::VoltageSource< myMatrixType > test(new object::LookupObj2D<double>( data, measurementpointRow, measurementpointColumn ) );
    myMatrixType current;
    current.zeros(1,10);
...
void TestElectricalElement::testElectricalParllelRCCreation()
{
    ...
    TS_ASSERT_EQUALS( stateSystemGroup.mDglStateSystem.GetEquationSystemAMatrix()(0, 1), symbolic::Symbolic("DIV(MUL(InputCurrent,ID12_ObjBase),ID12_ObjTau)") );
    TS_ASSERT_EQUALS( stateSystemGroup.mDglStateSystem.GetEquationSystemAMatrix()(0, 0), symbolic::Symbolic("NEG(DIV(1.000000,ID12_ObjTau))") );
...
void TestElectricalElement::testElectricalElementParallelRCPower()
{
    ...
    electrical::ParallelRC< myMatrixType > test(new object::ConstObj<double>( Resistance ), new object::ConstObj<double>( 1.0 / Resistance ) );
    test.SetCurrent(current);
    test.GetVoltage();

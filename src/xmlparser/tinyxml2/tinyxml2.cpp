		...
		*_end = 0;
		...
		if( _flags )
		{
			...
			while( p < _end )
			{
				...
				if( ( _flags & NEEDS_NEWLINE_NORMALIZATION ) && *p == LF )
				{
					if( *( p + 1 ) == CR )
						p += 2;
					else
				...
				if( ( _flags & NEEDS_ENTITY_PROCESSING ) && *p == '&' )
				{
...
#pragma warning (pop)
	...
	if( XMLUtil::StringEqual( p, cdataHeader, cdataHeaderLen ) )
	{
	...
	if( XMLUtil::StringEqual( p, elementHeader, elementHeaderLen ) )
	{
	...
	returnNode->_lineNumber = GetLineNumber( p );
                          ...
                          StrPair* parentEnd )
{
	...
	while( p && *p )
	{
		...
		if( !p )
		{
			...
			node = 0;
			if( !_document->Error() )
				_document->SetError( XML_ERROR_PARSING, 0, 0, _document->GetLineNumber( p ) );
			break;
		}
		...
		if( ele )
		{
			...
			{
				_document->SetError( XML_ERROR_MISMATCHED_ELEMENT, node->Value(), 0, _document->GetLineNumber( p ) );
				p = 0;
			}
			else
			if( !endTag.Empty() && ele->ClosingType() != XMLElement::OPEN )
                          ...
                          StrPair* )
{
	...
	if( this->CData() )
	{
		...
		if( !p )
			_document->SetError( XML_ERROR_PARSING_CDATA, start, 0, _document->GetLineNumber( p ) );
		return p;
	}
	else
	{
		...
		if( !p )
			_document->SetError( XML_ERROR_PARSING_TEXT, start, 0, _document->GetLineNumber( p ) );
		if( p && *p )
			return p - 1;
	}
	return 0;
                             ...
                             StrPair* )
{
	...
	if( p == 0 )
		_document->SetError( XML_ERROR_PARSING_COMMENT, start, 0, _document->GetLineNumber( p ) );
	return p;
}
                                 ...
                                 StrPair* )
{
	...
	p = _value.ParseText( p, "?>", StrPair::NEEDS_NEWLINE_NORMALIZATION );
	if( p == 0 )
		_document->SetError( XML_ERROR_PARSING_DECLARATION, start, 0, _document->GetLineNumber( p ) );
	return p;
}
                             ...
                             StrPair* )
{
	...
	p = _value.ParseText( p, ">", StrPair::NEEDS_NEWLINE_NORMALIZATION );
	if( !p )
		_document->SetError( XML_ERROR_PARSING_UNKNOWN, start, 0, _document->GetLineNumber( p ) );
	return p;
}
...
char* XMLElement::ParseAttributes( char* p )
{
	...
	while( p )
	{
		...
		if( !p || !( *p ) )
		{
			_document->SetError( XML_ERROR_PARSING_ELEMENT, start, Name(), _document->GetLineNumber( start ) );
			return 0;
		}
		...
		if( XMLUtil::IsNameStartChar( *p ) )
		{
			...
			if( !p || Attribute( attrib->Name() ) )
			{
				...
				_document->SetError( XML_ERROR_PARSING_ATTRIBUTE, start, p, _document->GetLineNumber( start ) );
				return 0;
			}
		...
		{
			_document->SetError( XML_ERROR_PARSING_ELEMENT, start, p, _document->GetLineNumber( p ) );
			return 0;
		}
	}
	return p;
...
XMLError XMLDocument::SaveFile( const char* filename,
                                bool        compact )
{
	FILE* fp = 0;
#if defined( _MSC_VER ) && ( _MSC_VER >= 1400 )
...
XMLError XMLDocument::SaveFile( FILE* fp,
                                bool  compact )
{
...
size_t XMLDocument::GetLineNumber( const char* currentchar )
{
	size_t charnumber = 0;

	if( currentchar > _charBuffer )
		charnumber = currentchar - _charBuffer;

	size_t lineNumber;
	for( lineNumber = 0; lineNumber < _line2charMapping.size(); ++lineNumber )
		if( _line2charMapping.at( lineNumber ) > charnumber )
			break;

	return lineNumber + 1;
}

void XMLDocument::CreateLineMappingVector( size_t length )
{
	for( size_t i = 0; i < length; ++i )
		if( _charBuffer[i] == '\n' )
			_line2charMapping.push_back( i + 1 );
}

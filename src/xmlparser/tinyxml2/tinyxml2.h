...
#define TIXMLASSERT( x ) \
    {                    \
    }    // if( !( x ) ) { __debugbreak(); } //if ( !(x)) WinDebugBreak()
...
class DynArray
{
    ...
    int Capacity() const { return _allocated; }

    const T* Mem() const { return _mem; }
...
class MemPoolT : public MemPool
{
    ...
    void Trace( const char* name )
    {
        printf( "Mempool %s watermark=%d [%dk] current=%d size=%d nAlloc=%d blocks=%d\n", name, _maxAllocs,
                _maxAllocs * SIZE / 1024, _currentAllocs, SIZE, _nAllocs, _blockPtrs.Size() );
    }
...
class XMLUtil
{
    ...
    inline static bool StringEqual( const char* p, const char* q, int nChar = INT_MAX )
    {
        ...
        if ( p == q )
            return true;
        // while( *p && *q && *p == *q && n < nChar ) // Original line
        while ( *p && *q && tolower( *p ) == tolower( *q ) &&
                n < nChar )    // HACK to make every lookup case insensitve this breaks UTF-8
        {
            ++p;
            ++q;
            ++n;
...
class XMLNode
{
    ...
    /// Get the XMLDocument that owns this XMLNode.
    XMLDocument* GetDocument() { return _document; }
    ...
    /** The meaning of 'value' changes for the specific type.
       ...
       @endverbatim
     */
    const char* Value() const { return _value.GetStr(); }
...
class XMLAttribute
{
    ...
    const char* Name() const { return _name.GetStr(); }
...
class XMLDocument : public XMLNode
{
    ...
    void SetError( XMLError error, const char* str1, const char* str2, const size_t linenumber = 0 );

    size_t GetLineNumber( const char* currentchar );
    ...
    size_t GetErrorLineNumber() const { return _lineNumber; }
    ...
    void CreateLineMappingVector( size_t length );
    ...
    char* _charBuffer;
    size_t _lineNumber;
    std::vector< size_t > _line2charMapping;
